...
#Constantes en rapport avec le fond de carte  (NOUVEAU THOMAS)
...
taillePointVillage=5
coefPointVillage=10

#Fichiers nécessaires afin de générer la carte
dependances = ["fondCarte1.txt", "fondCarte2.txt", "fondCarte3.txt", "script.js", "style.css"]
...
def CreerPanneauxInformation():
    ...
    for v in villages:
        ...
        textDow = CreerTexte("", x + 15, y + espacement, '<tspan class="avantValeur">Affichage : </tspan>', "txtRect","")
        f.write(textDow)
        ...
        textPop = CreerTexte("", x + 15, y + 2*espacement, '<tspan class="avantValeur">Population : </tspan>' + str(villages[v].population), "txtRect")
        f.write(textPop)
        
        for i in range(len(villages[v].categories)):
            text = CreerTexte("", x + 15, y + espacement +espacement*(i+2), '<tspan class="avantValeur">' + villages[v].categories[i].nom + ' : </tspan>' + villages[v].categories[i].commentaire, "txtRect")
            f.write(text)
...
def CreerPointVillage(nom, latitude, longitude, population, maxpop, fillColor, strokeColor):
    ...
    r = taillePointVillage+coefPointVillage*float(population)/maxpop # MODIF PAR THOMAS
...
def Cartographier(lien1, lien2, nfond, Name="carte"):
    ...
    global largeur_de_la_carte   
    global hauteur_de_la_carte
    global latitude_max_niger
    global latitude_min_niger
    global longitude_max_niger
    global longitude_min_niger
    global taillePointVillage
    global coefPointVillage
    ...
    for i in range(len(categories)):
        if( banListe[i] == 0 ):
            value = int(i/2) if i%2==0 else int(i+len(categories)/2)
    ...
    if nfond=='1':
        ...
        f.write('<div style="position: fixed;right: 1px;bottom: 1px;"><svg fill="#d2aa6d" stroke="#ffffff" height="50px" width="110px"><line x1="1" y1="15" x2="1" y2="35" class="echelleLine"></line><line x1="0" y1="25" x2="100" y2="25" class="echelleLine"></line><line x1="100" y1="15" x2="100" y2="35" class="echelleLine"></line></svg><span id="echelle" valeurBase="5" x="45" y="20" style="position:fixed;right:45px;">5 km</span></div>')
    else:
        f.write('<div style="position: fixed;right: 1px;bottom: 1px;"><svg fill="#d2aa6d" stroke="#ffffff" height="50px" width="110px"><line x1="1" y1="15" x2="1" y2="35" class="echelleLine"></line><line x1="0" y1="25" x2="100" y2="25" class="echelleLine"></line><line x1="100" y1="15" x2="100" y2="35" class="echelleLine"></line></svg><span id="echelle" valeurBase="5" x="45" y="20" style="position:fixed;right:45px;">5 km</span></div>')
    ...
    #On ajoute le fond de carte (MODIF PAR THOMAS)
    #print(nfond)
    if nfond=='2':
        fondCarte = open("fondCarte2.txt", 'r', encoding='utf8')
        #print("PSFIHJMSFMFSJPHOQFPFQSP")
        f.write('<svg id="carte" fill="#d2aa6d" stroke="#ffffff" height="100%" width="100%">\n<g id="zoom" transform="matrix(0.4,0,0,0.4,-100,-300)">\n')
        f.write(fondCarte.read())
        fondCarte.close()
        largeur_de_la_carte = 3273
        hauteur_de_la_carte = 3107
        latitude_max_niger = 14.027-0.074
        latitude_min_niger = 13.478+0.0015
        longitude_max_niger = 4.61+0.0022
        longitude_min_niger = 4.141-0.027
        taillePointVillage=10
        coefPointVillage=15
        #f.write('<div style="position: fixed;right: 1px;bottom: 1px;"><svg fill="#d2aa6d" stroke="#ffffff" height="50px" width="110px"><line x1="1" y1="15" x2="1" y2="35" class="echelleLine"></line><line x1="0" y1="25" x2="100" y2="25" class="echelleLine"></line><line x1="100" y1="15" x2="100" y2="35" class="echelleLine"></line></svg><span id="echelle" valeurBase="10" x="45" y="20" style="position:fixed;right:45px;">10 km</span></div>')

    elif nfond=='3':
        fondCarte = open("fondCarte3.txt", 'r', encoding='utf8')
        f.write('<svg id="carte" fill="#d2aa6d" stroke="#ffffff" height="100%" width="100%">\n<g id="zoom" transform="matrix(0.5,0,0,0.5,-100,-300)">\n')
        f.write(fondCarte.read())
        fondCarte.close()
        largeur_de_la_carte = 3273
        hauteur_de_la_carte = 3107
        latitude_max_niger = 13.976
        latitude_min_niger = 13.348
        longitude_max_niger = 4.7-0.007
        longitude_min_niger = 4.141-0.13
        taillePointVillage=10
        coefPointVillage=15
        #f.write('<div style="position: fixed;right: 1px;bottom: 1px;"><svg fill="#d2aa6d" stroke="#ffffff" height="50px" width="110px"><line x1="1" y1="15" x2="1" y2="35" class="echelleLine"></line><line x1="0" y1="25" x2="100" y2="25" class="echelleLine"></line><line x1="100" y1="15" x2="100" y2="35" class="echelleLine"></line></svg><span id="echelle" valeurBase="10" x="45" y="20" style="position:fixed;right:45px;">10 km</span></div>')

    else:
        fondCarte = open("fondCarte1.txt", 'r', encoding='utf8')
        f.write('<svg id="carte" fill="#d2aa6d" stroke="#ffffff" height="100%" width="100%">\n<g id="zoom" transform="matrix(0.745058 0 0 0.745058 -4321.54 -12000.9)">\n')
        f.write(fondCarte.read())
        fondCarte.close()
        #échelle de distance
        #f.write('<div style="position: fixed;right: 1px;bottom: 1px;"><svg fill="#d2aa6d" stroke="#ffffff" height="50px" width="110px"><line x1="1" y1="15" x2="1" y2="35" class="echelleLine"></line><line x1="0" y1="25" x2="100" y2="25" class="echelleLine"></line><line x1="100" y1="15" x2="100" y2="35" class="echelleLine"></line></svg><span id="echelle" valeurBase="10" x="45" y="20" style="position:fixed;right:45px;">10 km</span></div>')
    ...
    for v in villages:
        ...
        if len(villages[v].categories) > 0:
            ...
            CreerPointVillage(villages[v].nom,
                              ...
                              "white",
                              "black")
        #Si la population est supérieure à 1000 le nom du village sera affiché de manière permanente
    ...
    #Construction d'un tableau associant les villages au IDs pour JS.  (NOUVEAU)
    
    tableauNomVillage='{'
    for v in villages:
        tableauNomVillage += '"%s":%d,' % (villages[v].nom,v)
    tableauNomVillage = tableauNomVillage[:-1]
    tableauNomVillage += '}'
    
    #Construction d'un tableau associant les populations aux villages pour JS.  (NOUVEAU)
    
    tableauPopVillage='{'
    for v in villages:
        tableauPopVillage += '%d:%d,'%(v,villages[v].population)
    tableauPopVillage = tableauPopVillage[:-1]
    tableauPopVillage += '}'
    
    #Tableau des commentaires par catégorie et par village (NOUVEAU)
    tableauCommentaires = '{'
    for v in villages:
        tableauCommentaires += '%d:' % v
        tableauCommentaires += '['
        cat=False
        for i in range(len(villages[v].categories)):
            cat=True
            tableauCommentaires += '"%s: %s",' % (villages[v].categories[i].nom,villages[v].categories[i].commentaire)
        if cat==True:
            tableauCommentaires = tableauCommentaires[:-1]
        tableauCommentaires += '],'
    tableauCommentaires = tableauCommentaires[:-1]
    tableauCommentaires += '}'
    ...
    texte = texte.replace('% tableauNomVillage %', tableauNomVillage)
    texte = texte.replace('% tableauCommentaires %', tableauCommentaires)
    texte = texte.replace('% tableauPopVillage %', tableauPopVillage)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        ...
        print('Utilisation : %s fichier1 fichier2 n°fondDeCarte' %fileName)
        print('         Ou : %s fichier1 fichier2 n°fondDeCarte "Nom de la carte"' % fileName)
        print()
        print('    Exemple : %s Activites.xlsx Population.xlsx 1 "Carte Dankassari"' % fileName)
    elif os.path.exists(sys.argv[1]) and os.path.exists(sys.argv[2]):
        if len(sys.argv) < 5:
            Cartographier(sys.argv[1], sys.argv[2],sys.argv[3])
        else:
            Cartographier(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
