...
var tableauNomVillage = % tableauNomVillage %
var tableauCommentaires = % tableauCommentaires %
var tableauPopVillage = % tableauPopVillage %
    ...
    if (button.id == "PV") {
        ...
        if (button.classList.contains("active")) {
            ...
            for (let i = 0; i < cercles.length; i++) {
                let id = cercles[i].parentElement.id.slice(0, -1);
                if (tableauNbrTravauxVillage[id] > 0) {
                    cercles[i].setAttribute("stroke", "white"); // tentative de résoudre problème travaux
...
function disdow(id) { //TOUT NOUVEAU ! 
	...
	{
		var value = tableauNomVillage[i];
		if(tableauNomVillage[i]==id)
		{
			archive.push(i)
		}
	}
	...
	for(var j in tableauPopVillage) //POSSIBILITE DE FAIRE PLUS COMPACT ... A VOIR.
	{
			...
			if(j==id)
			{
				var population="Population: " + valeur.toString()
	...
	for(var k in tableauCommentaires)
	{
		if(k==id)
		{
			for(var l in tableauCommentaires[k])
			{
				archive.push(tableauCommentaires[k][l])
			}
